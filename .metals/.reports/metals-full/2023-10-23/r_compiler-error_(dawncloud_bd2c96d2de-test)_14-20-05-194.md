  ...
  test("Generate an ED25519") {
        val x = (for {
            keyPair <- EitherT.right(IO(new OctetKeyPairGenerator(Curve.Ed25519)
             ...
             privateKey <- EitherT.rightT(keyPair.toOctetKeyPair().toPrivateKey().asInstanceOf[java.security.interfaces.ECPrivateKey])
             json1 <- EitherT(IO(parse(keyPair.().toJSONString())))@@
             json <- EitherT(IO(parse(keyPair.toPublicJWK().toJSONString())))
             _ <- EitherT.right(IO.println(json.spaces2))
            ...
            // key <- EitherT((getECPublicKeyFromBase58(b58)))
             ...
             _ <- EitherT.right(IO.println(s"Signed ${sig} "))

             valid <- EitherT.rightT(validateSignature(sig,key.asInstanceOf[java.security.interfaces.ECPublicKey]))
