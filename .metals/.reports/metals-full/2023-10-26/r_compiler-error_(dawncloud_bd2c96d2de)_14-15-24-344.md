...
parent      = new OctetKeyPair.Builder(Curve.Ed25519,
  Base64Url(publicKey).build() KeyType null) # -1,
parent span = <4929..5057>,
child       = Base64Url(publicKey).build() KeyType null # -1,
child span  = [4968..5014..5064]
  ...
  def fromPublicKey(publicKey: String): KeyType =
    publicKey match
      case s: String if s.startsWith("MCowBQYDK2VwAyEA")                             =>
        val publicKeyBytes = java.util.Base64.getUrlDecoder.decode(publicKey)

        val keyGen          = new OctetKeyPairGenerator(Curve.Ed25519)
        val keyPair         = new OctetKeyPair.Builder(Curve.Ed25519,Base64Url(p@@ublicKey).build()
