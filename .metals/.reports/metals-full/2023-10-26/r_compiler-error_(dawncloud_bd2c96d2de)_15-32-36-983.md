...
parent      = new OctetKeyPair.Builder(Curve.Ed25519,
  Base64URL(pubKey.getEncoded).build() KeyType null) # -1,
parent span = <5230..5340>,
child       = Base64URL(pubKey.getEncoded).build() KeyType null # -1,
child span  = [5269..5314..5347]
    ...
    publicKey match
      case s: String if s.startsWith("MCowBQYDK2VwAyEA")                             =>
       ...
       // val subjectPublicKeyInfo = SubjectPublicKeyInfo.getInstance(publicKeyBytes)
        val keyPair         = new OctetKeyPair.Builder(Curve.Ed25519,Base64URL(pubKey@@.getEncoded).build()
